...
def cmp_text_assemble_timestamp():
    ...
    Date1 = Dow + sep + Mon + sep + DD + sep + HH + \
        sep + MM + sep + SS + sep + TZ + sep + YYYY
    Date2 = Dow + sep + HH + sep + MM + sep + \
        SS + sep + DD + sep + Mon + sep + YYYY
...
# compare the two json string representations - from strings in memory,
# not from files
...
def check_file(
    ...
    if cmp not in file_comparators:
        raise ValueError("file comparator %s not known" % str(cmp))

    # Do the comparison
    try:
        r = file_comparators[cmp](
            name,
            ref,
            msg=msg,
            quiet=quiet,
