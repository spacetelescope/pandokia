...
# If this is decided to be used again, please properly update the code as
# the database schema along with other scripts have changed.
#
...
from pandokia.utils.text_table import text_table
...
if False:
    ...
    for i in range(0, 100):
        pdk_db.execute("INSERT INTO user_prefs VALUES ( :1 , :2 )",
                       ('name_' + str(i), 'email_' + str(i)))
    pdk_db.commit()
...
# For projects that a user is interested in, find the name/format/maxlines
# preferences
...
def project_test_run(test_run, project):
    ...
    c = pdk_db.execute(
        "SELECT DISTINCT host, context FROM result_scalar WHERE test_run = :1 AND project = :2 ORDER BY host, context ",
        (test_run,
         project))
    host_context = []
...
# access dictionary with test_summary[(test_run,project)][host][status]
# host can be specific or all
...
def get_test_summary(test_run, project):
    ...
    for status, context, host in res:
        ...
        sum_dict[host][context][status] = sum_dict[
            host][context].get(status, 0) + 1

    for host in list(sum_dict.keys()):
        for context in list(sum_dict[host].keys()):
            for status in list(sum_dict[host][context].keys()):
                all_hosts[status] = all_hosts.get(
                    status, 0) + sum_dict[host][context][status]
    ...
    for host in list(sum_dict.keys()):
        if host == 'All':
            ...
            for context in list(sum_dict[host].keys()):
                sum_dict[host][context]['T'] = sum(
                    sum_dict[host][context].values())
...
def create_summary(test_run, project):
    ...
    hosts = sorted(test_summary.keys())
    ...
    for i, host in enumerate(hosts):
        if host != 'All':
            contexts = sorted(test_summary[host].keys())
    ...
    for project in projects:
        ...
        if format == 'c':
            ...
            if 0:
                ...
                #
                r = pandokia.contact_notify.get_contact_report(
                    username, project, test_run)
                if r is not None:
                    email += "Project: " + project + "\n\n"
                    email += pandokia.contact_notify.get_contact_report(
                        username, project, test_run)
                    email += '\n'
                    send_notice = True
                continue
        ...
        if format.capitalize() == 'F':
            ...
            # test run.
            anything, all_hosts, some_hosts = build_report_table(
                test_run, project, maxlines)
...
def build_report_table(test_run, project, maxlines):
    ...
    if all_hosts_more_count > 0:
        all_hosts.set_value(
            all_hosts.get_row_count(),
            'Test Name',
            '%d more' %
            all_hosts_more_count)

    if some_hosts_more_count > 0:
        some_hosts.set_value(
            some_hosts.get_row_count(),
            'Test Name',
            '%d more' %
            some_hosts_more_count)
