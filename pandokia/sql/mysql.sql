...
CREATE TABLE result_scalar (
	...
	test_hash VARCHAR(64)
		-- string of length 32 generated by md5; input is from test_run +
		-- project + host + test_name + context + custom
	);

-- Since the original unique index is ( test_run_type, project, host, test_name, context ),
-- and the combination of this five fields exceeds the index length limit of InnoDB and causes
-- integrity error, therefore, a hash of the combination is to replace the original without
-- exceeding the InnoDB limit
...
--	test_run_type+project+host+test_name+context+custom in order to
