...
def run():
    ...
    if 'MSIE' in os.environ['HTTP_USER_AGENT']:
        output.write(
            '<p>Internet Explorer fumbles the redirect.  Click the link below.</p>')
        no_redirect = 1
    else:
        no_redirect = 0
    ...
    if 'action_remove' in form:
        ...
        for key_id in valid_key_ids(form):
            pdk_db.execute(
                'DELETE FROM query WHERE qid = :1 AND key_id = :2 ', (qid, key_id))
        pdk_db.commit()
    ...
    if ('action_cattn' in form) or ('action_sattn' in form):
        ...
        for key_id in valid_key_ids(form):
            pdk_db.execute(
                "UPDATE result_scalar SET attn = :1 WHERE key_id = :2 ", (value, key_id))
        pdk_db.commit()
        ...
        # we don't need to validate client because it was done already in the
        # main program
        ...
        pdk_db.execute(
            "UPDATE query_id SET username = :1 WHERE qid = :2 ",
            (common.current_user(),
             qid))
        pdk_db.commit()
        ...
        if v:
            ...
            if 0:
                s = "SELECT DISTINCT result_scalar.test_run FROM result_scalar, query WHERE query.qid = %d and result_scalar.key_id = query.key_id " % (
                    qid, )
                print("S=%s<br>" % s)
                c = pdk_db.execute(s)
                for x in c:
        ...
        pdk_db.execute(
            "UPDATE query_id SET username = :1, expires = :2 WHERE qid = :3",
            (common.current_user(),
             expire,
             qid))
...
def copy_qid(old_qid):
    ...
    s = "INSERT INTO query ( qid, key_id ) SELECT %d, key_id FROM query WHERE qid = %d" % (
        new_qid, old_qid)
    pdk_db.execute(s)
    pdk_db.commit()
