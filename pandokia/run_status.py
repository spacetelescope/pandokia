...
if platform.system() == 'Windows':
    ...
    class status_block(object):
        ...
        def get_status_text(self, n):
            return self.get_value_at_offset(
                n, self.status_text_offset, self.status_text_size)
        ...
        def get_value_at_offset(self, n, offset, len):
            ...
            # if the valid flag changed while we were reading data, we can't
            # read the value
        ...
        def set_my_record(self, n):
            if n >= self.n_records:
                raise Exception(
                    'only %d records in file - using #%d\n' %
                    (self.n_records, n))
            self.my_record = n
            self.my_record_offset = self.header_size + n * self.record_size

        def set_status_text(self, value):
            return self.set_value_at_offset(
                self.status_text_offset, self.status_text_size, value)
        ...
        def set_value_at_offset(self, offset, blocklen, value):
            ...
            try:
                self.mem[
                    start: start +
                    self.valid_flag_size] = self.locked_valid_flag
            except TypeError:
                self.mem[
                    start: start +
                    self.valid_flag_size] = bytes(
                    self.locked_valid_flag,
                    'ascii')
            ...
            try:
                self.mem[
                    start: start + self.valid_flag_size] = "%*d" % (self.valid_flag_size, n)
            except TypeError:
                self.mem[
                    start: start +
                    self.valid_flag_size] = bytes(
                ...
                if s is None:
                    ...
                    try:
                        ...
                        if tyme not in times:
                            times = {}
                            times[tyme] = str(
                                datetime.datetime.fromtimestamp(tyme))
                        tyme = times[tyme]
                        text = str(text).strip()
                        if text != '':
                            any = 1
