...
def t000_create():
    dbx.execute(
        'create table test_table ( a varchar(100), b varchar(100), c integer )')
...
def t004_insert():
    ...
    for x in data_set:
        dbx.execute(
            'insert into test_table ( a, b, c ) values ( :1, :2, :3 )', x)
    dbx.commit()
...
def t005_select_1():
    c = dbx.execute(
        'select a, b, c from test_table where a = :1 and b = :2', ('a', 'b'))
    l = [x for x in c]
    print(('result', l))
    assert l == [('a', 'b', 1)]
...
def t005_select_2():
    c = dbx.execute(
        'select a, b, c from test_table order by a asc, b asc, c asc',
        None)
    l = [x for x in c]
    print(('result', l))
...
def t030_query_with_where():
    ...
    cur = dbx.execute(
        "select a, b, c from test_table %s" %
