...
def add_project(user):
    ...
    output.write('<br>')
    output.write(
        '<a href=%s?query=prefs>Back to preference page</a>' %
        common.get_cgi_name())
...
def save(user):
    ...
    for project in form.getlist('projects'):
        ...
        # delete and insert instead of update because we may be adding new
        # projects.
        cfg.pdk_db.execute(
    ...
    output.write(
        '<a href=%s?query=prefs>Back to preference page</a><br>' %
        common.get_cgi_name())
    output.write(
        '<a href=%s>Back to top level</a><br>' %
        common.get_cgi_name())
